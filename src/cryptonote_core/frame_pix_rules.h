...
namespace frame_pixs {
  // Frame pix decommissioning: as service nodes stay up they earn "credits" (measured in blocks)
  ...
  constexpr int16_t CHECKPOINT_NUM_QUORUMS_TO_PARTICIPATE_IN = 2;
  constexpr int16_t CHECKPOINT_MAX_MISSABLE_VOTES            = 1;
...
#ifndef GUUS_ENABLE_INTEGRATION_TEST_HOOKS
  ...
  constexpr std::array<int, 3> MIN_GUUSNET_VERSION{{0, 7, 0}};

  // The minimum accepted version number, broadcasted by Frames via uptime proofs for each hardfork
  ...
  constexpr proof_version MIN_UPTIME_PROOF_VERSIONS[] = {
    {cryptonote::network_version_15,                  {7,1,2}},
    {cryptonote::network_version_14,                {6,1,0}},
    {cryptonote::network_version_13,  {5,1,0}},
    {cryptonote::network_version_12,        {4,0,3}},
  };
  ...
  {
    return
        hf_version <= cryptonote::network_version_12 ? quorum_type::obligations :
...
// return: UINT64_MAX if (num_contributions > the max number of contributions), otherwise the amount in guus atomic units
...
bool check_frame_pix_portions(uint8_t version, const std::vector<uint64_t>& portions);
