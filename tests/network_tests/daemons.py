...
class Daemon(RPCDaemon):
    ...
    def __init__(self, *,
        ...
        if name is None:
            name = 'guusd@{}'.format(self.rpc_port)
        super().__init__(name)
        ...
        self.args = [guusd] + list(self.__class__.base_args)
        self.args += (
                '--data-dir={}/guus-{}-{}'.format(datadir or '.', self.listen_ip, self.rpc_port),
                '--log-level={}'.format(log_level),
                '--log-file=guus.log'.format(self.listen_ip, self.p2p_port),
    ...
    def ping(self, *, storage=True, guusnet=True):
        """Sends fake storage server and guusnet pings to the running guusd"""
        if storage:
            self.json_rpc("storage_server_ping", { "version_major": 9, "version_minor": 9, "version_patch": 9 })
        if guusnet:
            self.json_rpc("guusnet_ping", { "version": [9,9,9] })
